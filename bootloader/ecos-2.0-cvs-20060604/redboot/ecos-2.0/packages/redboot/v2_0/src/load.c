...
#ifdef KEN
//
// Read the next data block from the stream.
// Returns:
//    > 0 - number of data bytes
//      0 - error or EOF, status in getc_info.err
//
static int
redboot_getblock(unsigned char *mp)
{
    static char spin[] = "|/-\\|-";
    if (getc_info.avail < 0) {
      return -1;
    ...
    if (getc_info.avail == 0) {
        ...
        getc_info.bufp = getc_info.buf;
        getc_info.len = (*getc_info.fun)(mp, BUF_SIZE, &getc_info.err);
        if ((getc_info.avail = getc_info.len) <= 0) {
            if (getc_info.len < 0) {
...
do_load(int argc, char *argv[])
{
    ...
    if (raw) {
        ...
        err = 0;
#ifdef KEN 
         while ((res = redboot_getblock(mp)) > 0) {
 	  mp += res;
 	}
#else   
