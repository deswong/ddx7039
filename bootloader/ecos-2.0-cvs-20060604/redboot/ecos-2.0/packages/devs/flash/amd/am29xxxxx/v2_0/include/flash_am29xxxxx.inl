...
flash_query(void* data)
{
    ...
    ROM = (flash_data_t*) CYGNUM_FLASH_BASE;  //6000000
  

    f_s1 = FLASH_P2V(ROM+FLASH_Setup_Addr1); //555
    f_s2 = FLASH_P2V(ROM+FLASH_Setup_Addr2); //2A
   *f_s1 = FLASH_Reset;                      //F0

    w = *(FLASH_P2V(ROM));

    *f_s1 = FLASH_Setup_Code1;  //AA
    *f_s2 = FLASH_Setup_Code2;  //55
    *f_s1 = FLASH_Read_ID;      //90

    id[0] = -1;
    id[1] = -1;

    // Manufacturers' code
    id[0] = *(FLASH_P2V(ROM+FLASH_VendorID_Addr));   //0
    // Part number
    id[1] = *(FLASH_P2V(ROM+FLASH_DeviceID_Addr));    //1
    id[2] = *(FLASH_P2V(ROM+FLASH_DeviceID_Addr2));   //0e
    id[3] = *(FLASH_P2V(ROM+FLASH_DeviceID_Addr3));    //0f
